...
"""
目的
base model(LightGBM) + 特徴量エンジニアリング(波長マイナス, redshift*スペクトルの差分) + カテゴリカルデータの削除 + alpha * delta + 天球座標の周期性の追加
...
21     sin_delta        1927
11           i_z        1631
13  redshift_g_r        1560
4              r        1464
19     sin_alpha        1387
5              i        1368
20     cos_alpha        1332
14  redshift_r_i        1153
12  redshift_u_g        1117
15  redshift_i_z        1117
17     alpha_rad           0
18     delta_rad           0

分かったこと
- 天球座標の角度の周期性
  - 周期性の追加は予測精度が悪化したため追加しない方が良い
  - alpha_rad = alpha * π / 180 より alpha と一致するため使用されない

次回やること
- alpha と delta の特徴量の組み合わせを増やしてみる
  - 物理的な意味を考えるのが良さそう
    - 比率特徴量(u / g, g / r, r / i, i / z)
    - redshift との比率(redshift / g_r, redshift / u_g)
    - alpha * redshift, delta * redshift
"""
...
def feat_eng_celestial_coordinate(
    train_pl_df: pl.DataFrame, test_pl_df: pl.DataFrame
) -> tuple[pl.DataFrame, pl.DataFrame]:
    """天球座標の周期性を新しい特徴量として追加する関数
    ...
    Returns:
        tuple[pl.DataFrame, pl.DataFrame]: 天球座標の周期性を追加した train, test データ
    """
    df_list = [train_pl_df, test_pl_df]
    calc_df_list = []
    for df in df_list:
        calc_df_list.append(
            df.with_columns(
                [
                    (np.deg2rad(pl.col("alpha"))).alias("alpha_rad"),
                    (np.deg2rad(pl.col("delta"))).alias("delta_rad"),
                    (np.sin(pl.col("alpha"))).alias("sin_alpha"),
                    (np.cos(pl.col("alpha"))).alias("cos_alpha"),
                    (np.sin(pl.col("delta"))).alias("sin_delta"),
                    (np.cos(pl.col("delta"))).alias("cos_delta"),
                ]
            )
        )
...
def main() -> None:
    ...
    train_pl_df, test_pl_df = feat_eng_celestial_coordinate(train_pl_df, test_pl_df)
