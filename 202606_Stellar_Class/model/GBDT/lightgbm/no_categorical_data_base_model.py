...
"""
目的
base model(LightGBM) + 特徴量エンジニアリング(波長マイナス, redshift*スペクトルの差分) + カテゴリカルデータの削除
...
4              r        1873
5              i        1741
13  redshift_g_r        1607
12  redshift_u_g        1404
14  redshift_r_i        1383
15  redshift_i_z        1369

分かったこと
- catboost よりも LightGBM の方が予測精度が 0.008 高い
- 特徴量重要度が大きく異なる
  - 分岐で重要視している特徴量が異なる
- catboost も LightGBM も redshift を重要視している

次回やること
- これからは LightGBM を base model にする
  - 特徴量エンジニアリング
    - alpha_delta = alpha * delta
    - distance = np.sqrt(alpha**2 + delta**2)
    - sin(alpha), cos(alpha)
"""
