r"""Year の特徴量重要度が高いので，Year を drop して CV を確認する(idは0.0なので削除する)
thid の結果
特徴量重要度の確認
                   feature  importance
4                     Year   52.427425
...
考察
- LapTime_Delta と TyreLife の関係性
  - LapTime_Delta(基準と現在のラップタイムとの差) に変化があると TyreLife が寿命に近い？
- stint と TyreLife の関係性
  - stint の値が小さいとまだレースは序盤で TyreLife の寿命はまだ
- RaceProgress と TyreLife の関係性
  - RaceProgress が後半になるにつれて TyreLife との関係性は線形以下(pairplot参照)になる
- LapNumber が低いのは, RaceProgress, Stint, TyreLife と重複しているから？
- Driver は意外に低い
  - 車の状態に関係してそう

結論
一応, Year は予測精度に貢献しているから削除するわけにはいかない

次にやること
下の特徴量を一つずつ追加して，5-fold-CV を行う
- 作成する特徴量
  1. DegradationPerTyreLife = Cumulative_Degradation / (TyreLife + 1)
